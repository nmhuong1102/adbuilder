$(document).ready(function() {
    var $body = $("body"),
        sampleData = "db/sample.json";

    var renderFormByJson = function(json) {
        ...
        for(var i = 1; i <= 4; i++) {
            ...
            console.log(products["product" + i]["clickTag" + i]);
            $("#clickTag" + i + " input.form-control").val(products["product" + i]["clickTag" + i]);
        }
    }

    $.getJSON(sampleData, function(json) {
        renderFormByJson(json);
        // renderJsonToAdInfo(json);
        // adInfoFilled(json);
    });

    $body
    .on("change, keyup", "#brandName .form-control", function() {
